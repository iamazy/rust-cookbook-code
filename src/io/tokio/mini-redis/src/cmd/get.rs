...
/// 
/// If the key does not exist the special value nil is returned. An error
/// is returned if the value stored at key is not a string, because GET only
...
impl Get {
    ...
    /// the `Frame`. At this point, the entire frame has already been received from
    /// the socket.
    /// 
    ...
    pub(crate) fn parse_frames(parse: &mut Parse) -> crate::Result<Get> {
        // The `GET` string has already been consumed. The next value is the name of
        // the key to get. If the next value is not a string or the input is fully consumed,
    ...
    pub(crate) async fn apply(self, db: &Db, dst: &mut Connection) -> crate::Result<()>{
        ...
        // Write the reponse back to the client
