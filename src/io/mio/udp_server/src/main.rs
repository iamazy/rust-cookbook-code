...
fn main() -> io::Result<()> {
    ...
    // Create storage for events. Since we will only register a single socket,
    // a capacity of 1 will do
    ...
    loop {
        ...
        for event in events.iter() {
            ...
            // in this example it will only ever be one but we make sure it's
            // valid none the less
            match event.token() {
                UDP_SOCKET => loop {
                    ...
                    match socket.recv_from(&mut buf) {
                        ...
                        Err(e) => {
                            // If it was any other kind of error,someting wen wrong and we terminate with an error.
                            return Err(e);
                        }
                    }
                },
